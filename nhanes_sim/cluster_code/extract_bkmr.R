...
ksm_triv <- names(ksm_paths)[14:17] |> # only for 3-way
  purrr::map_df(\(x) {
    ...
    ksm_paths[[x]] |> 
      purrr::map_df(\(y) {
        bkmr <- read_rds(y)
        result <- trivarsurf_bkmr(bkmr, 13, 9, 8, 
...
ksm_ints <- names(ksm_paths)[2:17] |> 
  purrr::map_df(\(x) {
    message("starting ", x)
    indices <- case_when(
      x %in% 2:5 ~ list(c(13, 8)), # D05 and 194
      x %in% 6:9 ~ list(c(18, 15)), # F08 and F03
      x %in% 10:13 ~ list(c(1, 8)), # 074 & 194
      x %in% 14:17 ~ list(c(13, 9, 8)) # D05, PCB, 194
    )
    ksm_paths[[x]] |> 
      purrr::map_df(\(y) {
        bkmr <- read_rds(y)
...
# FDR for two-way
indices <- t(combn(1:18, 2))
ksm_intb_hgni <- names(ksm_paths)[2:13] |> # only for two-way
  purrr::map_df(\(x) {
    message("starting ", x)
    ksm_paths[[x]] |> 
      purrr::map_df(\(y) {
...
write_csv(ksm_intb_hgni, "sim/bkmr_sm/int_bivar_full.csv")
...
# need to get int_trivar_full for FDR three-way
# write function
get_3way_fdr_bk <- function(case) {
  # get names of files in 'mod' folder
  fs <- list.files("mods", full.names = T) 
  ...
  # all combinations of indices, excep 8 9 13 (815 ways!)
  combs <- combn(1:18, 3)
      ...
      xres <- fcombs |> 
        purrr::map_df(\(y) {
          ...
          ints <- trivarinter_bkmr(bkmr, 
                                   ...
                                   qs.fixed = c(0.25, 0.75), 
                                   q.rest = 0.5) 
          yres <- data.frame(ints) |> 
            mutate(comb = paste(y, collapse = " "))
          return(yres)
        }) |> 
        mutate(trial = as.numeric(sub(".*_(\\d+)\\.RDS", "\\1", x)))
      write_rds(xres, paste0("fdrs/", substr(x, nchar(x) - 8, nchar(x))))
      return(xres)
    }) |> 
    mutate(case = case)
  write_rds(result, "fdrs.RDS")
}
# send to hpc
for(i in 14:17) {
  slurm_call(
    get_3way_fdr_bk, 
    params = list(case = i), 
    global_objects = c('trivarinter_bkmr'),
    jobname = paste0('bkmr_sm', i))
}
...
write_csv(klg_intb, "sim/bkmr_lg/int_bivar_full.csv")
tictoc::toc()
        ...
        1:3 |> # for each stratified model
          purrr::map_df(\(z) {
            ...
            if(class(mod)[1] == "logical") { # if output failed
              ...
              df <- SingVarRiskSummaries(mod, which.z = 13, q.fixed = 0.5) |> 
                select(est, sd) |> 
                mutate(race = z)
            }
            # print(df)
...
# fdr's, get confidence intervals
ksmre_fdrs <- names(ksmre_paths) |> 
  purrr::map_df(\(x) { # for each case
    message("starting", x)
    ksmre_paths[[x]] |> 
      purrr::map_df(\(y) { # for each trial
        ...
        1:3 |> # for each stratified model
          purrr::map_df(\(z) {
            mod <- list_mod[[z]]
            if(class(mod)[1] == "logical") { # if output failed
              df <- data.frame(est = NA, sd = NA, race = z, variable = NA)
            } else { 
              df <- data.frame()
              for(i in c(1:12, 14:18)) { # for each chemical
                # compute estimate of 0.75 vs. 0.25 quantiles
                temp <- SingVarRiskSummaries(mod, which.z = i, q.fixed = 0.5) |> 
                  select(est, sd) |> 
                  mutate(race = z, variable = i)
...
write_csv(ksmre_fdrs, "sim/re/ksmre_ints.csv")
        ...
        1:3 |> # for each stratified model
          purrr::map_df(\(z) {
            ...
            if(class(mod)[1] == "logical") { # if output failed
              ...
              df <- PredictorResponseUnivar(mod, which.z = 13) |> 
                select(z1 = z, est, se) |> 
                mutate(race = z)
            }
            # print(df)
...
# klgre_paths2 <- Filter(function(x) !(length(x) == 0), klgre_paths)
# klgre_paths <- klgre_paths2
...
write_csv(klgre_fdrs, "sim/re/klgre_ints.csv")
...
klgre_patht2 <- Filter(function(x) !(length(x) == 0), klgre_patht)
klgre_patht <- klgre_patht2
