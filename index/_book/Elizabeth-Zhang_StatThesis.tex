...
{\centering \includegraphics[width=0.8\linewidth]{figures/ch4_cdasflowchart} 

}

\caption{cd as flow chart.}\label{fig:cdasflow}
\end{figure}
...
First, the base case scenario, with no interactions, confirms that BKMR and BSR behave as expected. We affirm the results of previous studies, which find that BKMR is able to flexibly capture complex additive exposure-response relationships (i.e., no interactions), though false discovery rates can be higher than other methods (\protect\hyperlink{ref-hoskovec_model_2021}{Hoskovec et al., 2021}; \protect\hyperlink{ref-lazarevic_performance_2020}{Lazarevic et al., 2020}). Similar to Pesenti et al. (\protect\hyperlink{ref-pesenti_comparative_2023}{2023}), we also find that BKMR generally has higher sensitivity than BSR, especially in smaller size datasets.
...
However, in cases where at least one of the chemicals participating in the interaction was not marginally significant, BKMR and BSR were generally not able to detect the interaction. {[}\emph{update with new analysis? then compare to past w/ interactions}{]}
...
These results extend the findings from (\protect\hyperlink{ref-presenti_comparative_2023}{\textbf{presenti\_comparative\_2023?}}) by providing a more comprehensive range of interactions and guidance for the use of BKMR and BSR for the detection of interactions. Moreover, to our knowledge, this is the first simulation study of interactions in exposure mixtures since those performed by sun and barrera. note improvements/changes. we also note that \emph{interactiosn dependent on univariate effects???}
\begin{itemize}
\tightlist
\item
