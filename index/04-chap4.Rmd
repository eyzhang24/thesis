...
comb_sens |> 
  ...
  kbl(booktabs = TRUE, digits = 2, #align = "lcc",
      caption = "Overall sensitivity for univariate chemicals in all scenarios with interactions between chemicals.", 
      col.names = c("Type", "Effect size", 
                    rep(c("Naive", "Oracle", "BKMR", "BSR"), 2))
      ) |> 
  add_header_above(header = c(" " = 2, "Small" = 4, "Large" = 4)) |>
...
re_ints <- bind_rows(
  ...
  mutate(effect_size = ifelse(case %in% c(1, 3), "Lower", "Higher"), 
         case = ifelse(case %in% c(1, 2), 
                       paste0("non-Hisp. black", footnote_marker_symbol(2)), 
                       paste0("Hisp. born outside US", footnote_marker_symbol(3)))
         ) |> 
...
re_ints |> kbl(booktabs = TRUE, escape = FALSE, 
  ...
  add_header_above(header = c(" " = 2, "Uncollapsed" = 2, 
                              "Collapsed*" = 1, 
                              " " = 2)) |> 
  add_header_above(header = c(" " = 2, "Small (n=250)" = 3, "Large (n=1000)" = 2), 
                   bold = TRUE) |>
  collapse_rows(columns = 1, valign = "middle", latex_hline = "linespace") |> 
  # column_spec(7, width = "6em") |>
  add_footnote(c("\"Collapsed\" refers to scenarios where the smallest three race categories are collapsed\ninto one stratified model.", "Original n=27", "Original n=109"), notation = "symbol")
```
...
```{r exprespre, fig.cap = "Relationship between Hg and response estimated by stratified BKMR and BSR models in smaller and larger datasets. All other chemicals are fixed at 0.5 quantiles.", echo = FALSE, out.width = '100%'}
# figure
...
Table \@ref(tab:runtimesre) summarizes the average run-times of the naive MLR, oracle MLR, and stratified BKMR in scenarios with an interaction between race and Hg. 

Models 

Again, we note that we were unable to get run-times for BSR within the timeframe of this thesis. However, based on observations of stratified BSR in the high-performance computing cluster, we estimate that it took around 20 hours for all stratified models on one smaller size dataset to run, and 15 hours for one larger size dataset. 
...
retimes |> 
  ...
  kbl(booktabs = TRUE, align = "llcccc", 
      ...
      col.names = c("Model", "Race", "Smaller size (n=", "Larger size")) |> 
  collapse_rows(columns = 1, valign = "middle", latex_hline = "linespace") |> 
  add_footnote("Original sample sizes of race category: Non-Hispanic white (n=16), Non-Hispanic\nblack (n=27), Non-Hispanic other (n=13), Hispanic born in US (n=87),\nHispanic born outside US (n=109), and Collapsed non-Hispanic (n=56=16+27+13)")
```
