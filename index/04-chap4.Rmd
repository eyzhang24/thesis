...
In general, the detection rates of univariate chemicals that participate in an interaction differ in comparison to the base case. First, we note that the sensitivity for univariate chemicals that participate in interactions is reduced for oracle MLRs, compared to the base case (Table \@ref(tab:onewaytabses) and Figures \@ref(fig:osmunivp) and \@ref(fig:olgunivp)). However, this pattern is unique to the oracle MLR, as they contain explicit interaction terms, so p-values for univariate terms that participate in an interaction are not interpretable. 
...
Now, for the naive MLR, BKMR, and BSR, chemicals that are marginally insignificant but participate in an interaction can occasionally be detected, depending on their form, effect size, and the sample size of the data. For instance, in scenarios with a higher effect polynomial interaction between As and Cd, both of which are univariately insignificant, the larger size naive MLR has a sensitivity of 0.67 for Cd (Figure \@ref(fig:nlgunivp)), BKMR 0.78, and BSR 0.55. In comparison, the sensitivities are all lower in scenarios with the higher effect multiplicative interaction between As and Cd. This is likely because, in the theoretical model for this scenario, Cd has a strong positive relationship with the health response at lower levels of As and a weak relationship at higher levels of As (Figure \@ref(fig:bp2)). On the other hand, the relationship between Cd and the health response inverts at lower versus higher levels of As (Figure \@ref(fig:bm2)), which would be more difficult to detect when only considering additive effects. This demonstrates that non-additive interactions may occasionally be detected through linear, univariate terms, depending on their form, and when the effect size and sample size are large enough.  

Chemicals that are marginally significant — in our case, Hg and Ni — and participate in an interaction may be detected slightly less often via their univariate effects. 

### Two-way interactions between chemicals

This section describes the ability of BKMR and BSR to detect two-way interactions between chemicals, using results from the oracle MLR as a baseline. 

First, we include the full output of 

In exposure mixture studies, a visual assessment of such diagnostic plots is often the first 
...
```{r exprespre, fig.cap = "Relationship between Hg and response estimated by stratified BKMR and BSR models in smaller and larger datasets. All other chemicals are fixed at 0.5 quan-les.", echo = FALSE, out.width = '100%'}
# figure
...
re_ints |> kbl(booktabs = TRUE, escape = FALSE, 
  ...
  add_footnote(c("\"Collapsed\" refers to scenarios where the smallest three race categories are collapsed into \none stratified model.", "Original n=27", "Original n=109"), notation = "symbol")
```
...
times |> 
  kbl(booktabs = TRUE, 
      align = "llcccc", 
      caption = "Run-times in all scenarios with interactions between two or more chemicals, as well as the base case.", 
...
retimes |> 
  ...
  kbl(booktabs = TRUE, align = "llcccc", 
      caption = "Run-times in scenarios with an interaction between the categorical race covariate and Hg.", 
      col.names = c("Model", "Race", "Smaller size", "Larger size")) |> 
  collapse_rows(columns = 1, valign = "middle", latex_hline = "linespace") |> 
  add_footnote("Original sample sizes of race category: Non-Hispanic white (n=16), Non-Hispanic black (n=27), Non-Hispanic other (n=13), Hispanic born in US (n=87), Hispanic born outside US (n=109), and Collapsed non-Hispanic (n=56=16+27+13)")
```
