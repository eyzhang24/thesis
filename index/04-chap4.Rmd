...
```{r exprespre, fig.cap = "diagnostic plot of bkmr, bsr RE.", echo = FALSE, out.width = '100%'}
# figure
...
re_ints <- bind_rows(
  ...
  mutate(effect_size = ifelse(case %in% c(1, 3), "Lower", "Higher"), 
         case = ifelse(case %in% c(1, 2), 
                       paste0("non-Hispanic black", footnote_marker_symbol(2)), 
                       paste0("Hispanic born outside US", footnote_marker_symbol(3)))
         ) |> 
...
retimes |> 
  ...
  kbl(booktabs = TRUE, align = "llcccc", 
      caption = "Run-times in scenarios with an interaction between the categorical race covariate and Hg. \"NH Bl\" and \"H b/o US\" represent scenarios in which the effect of Hg is increased in Non-Hisp. black and Hispanic born outside US categories, respectively.", 
      col.names = c("Model", "Race", "NH Bl", "H b/o US", "NH Bl", "H b/o US")) |> 
  add_header_above(header = c(" " = 2, "Small (n=252)" = 2, "Large (n=1000)" = 2)) |> 
  add_footnote("Original sample sizes of race category are as follows: Non-Hispanic white (n=16), Non-Hispanic black (n=27), Non-Hispanic other (n=13), Hispanic born in US (n=87), Hispanic born outside US (n=109), and Collapsed non-Hispanic (n=56=16+27+13)")
```
