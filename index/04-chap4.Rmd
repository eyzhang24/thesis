...
[*is it necessary to provide an equation?*]
...
Figure \@ref(fig:hgniuniv)a and \@ref(fig:hgniuniv)b display the distribution of univariate p-values and PIPs, respectively, in scenarios with an interaction between Hg and Ni, which are themselves univariately significant. See Figures \@ref(fig:nsmunivp)-\@ref(fig:slgunivp) in Appendix \@ref(suppresults) for similar output for all other scenarios with interactions between chemicals. Overall, the distributions look similar to the base case, though there are slight differences in some cases for chemicals that participate in interactions.
...
Table \@ref(tab:onewaytab) summarizes the sensitivity and false discovery rates of univariate chemicals in all scenarios with interactions between chemicals, comparing the form of interactions, effect sizes, size of datasets, and models. 
...
[*turn this page horizontal!! or figure out a way to stack the tables. or put fdr's in appendix!*]
...
comb_sens |> 
  ...
  kbl(booktabs = TRUE, digits = 2, #align = "lcc",
      caption = "Sensitivity and false discovery rate (FDR) of univariate chemicals in all scenarios with interactions between exposures.", 
